...
class PublicRecipeAPITests(TestCase):
    ...
    def test_auth_required(self):
        """Authentication is required to call API"""
...
class PrivateRecipeAPITests(TestCase):
    ...
    def setUp(self):
        self.client = APIClient()
        self.user = create_user(email="jane@example.com", password="jane12345")
        self.client.force_authenticate(self.user)

    def test_retrieve_recipes(self):
        """Test fetching a list of recipes"""
    ...
    def test_recipe_list_limited_to_user(self):
        ...
        other_user = create_user(
            email="john@example.com", password="john#11121"
        )  # noqa
    ...
    def test_update_user_returns_error(self):
        ...
        new_user = create_user(email="alice@example.com", password="1234#alice")  # noqa
        recipe = create_recipe(user=self.user)
    ...
    def test_delete_other_users_recipe_error(self):
        ...
        new_user = create_user(email="another@example", password="test#1231!")
    ...
    def test_create_recipe_with_existing_tags(self):
        ...
        tag_swahili = Tag.objects.create(user=self.user, name="Swahili")
        payload = {
            "title": "Pilau",
            "time_minutes": 30,
            "price": Decimal("10.24"),
            "tags": [{"name": "Swahili"}, {"name": "Lunch"}],
        }
        ...
        recipe = recipes[0]
        self.assertEqual(recipe.tags.count(), 2)
        self.assertIn(tag_swahili, recipe.tags.all())
        for tag in payload["tags"]:
            exists = recipe.tags.filter(
                name=tag["name"], user=self.user
            ).exists()  # noqa
    ...
    def test_create_an_ingredient_on_update(self):
    ...
    def test_update_recipe_assign_ingredient(self):
        ...
        second_ingredient = Ingredient.objects.create(
            user=self.user, name="cumin"
        )  # noqa
        payload = {"ingredients": [{"name": "cumin"}]}
        url = detail_url(recipe.id)
        res = self.client.patch(url, payload, format="json")
